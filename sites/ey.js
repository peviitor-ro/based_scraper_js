...
s.soup.then((response) => {
    const totalJobs = parseInt(response.find("span", { class: "paginationLabel" }).findAll("b")[1].text);
    const step = 25;
    ...
    const fetchData = () => {
        return new Promise((resolve, reject) => {
            for (let i = 0; i < range.length; i++) {
                let url = "https://careers.ey.com/ey/search/?q=Romania&startrow=" + range[i];
                const s = new scraper.Scraper(url);
                s.soup.then((response) => {
                    let jobs = response.find("table", { class: "searchResults" }).find("tbody").findAll("tr");
                    jobs.forEach((job) => {
                        const id = uuid.v4();
    ...
    fetchData().then((finalJobs) => {
      ...
      let logo = "https://rmkcdn.successfactors.com/bcfdbc8a/688bb7d2-e818-494b-967e-0.png";
