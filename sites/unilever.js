...
s.get().then((response) => {
    ...
    const jobs = scraper.soup(jobsData).find("ul", {class:"global-job-list--white"}).findAll("li");

    jobs.forEach((job) => {
        const id = uuid.v4();
        const job_title = job.find("a").findAll("span")[0].text.trim();
        const job_link = "https://careers.unilever.com" + job.find("a").attrs.href;
    ...
    let logo = "https://1000logos.net/wp-content/uploads/2017/06/Unilever-Logo-500x379.png";

    let postLogo = new scraper.ApiScraper(
      "https://api.peviitor.ro/v1/logo/add/"
    );
    postLogo.headers.headers["Content-Type"] = "application/json";
    postLogo.post(JSON.stringify([{ id: "Unilever", logo: logo }]));
});
